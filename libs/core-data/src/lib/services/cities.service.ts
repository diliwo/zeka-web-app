...
export class CitiesService {
  ...
  getAll(pageNumber: number, pageSize: number, filter:string= '',orderBy:string=""): Observable<Cities> {
        ...
        _observableCatch((response_: any) => {
          if (response_ instanceof HttpResponseBase) {
            try {
              ...
              return <Observable<Cities>>(<any>_observableCatch(e));
            }
          } else
            return <Observable<Cities>>(<any>_observableCatch(response_));
        })
      );
  }

  protected processGetAll(response: HttpResponseBase): Observable<Cities> {
    ...
    if (status === 200) {
      return servicesLib.blobToText(responseBlob).pipe(
        _observableMergeMap((_responseText) => {
          ...
          result200 = Cities.fromJS(resultData200);
    ...
    return _observableOf<Cities>(<any>null);
  }

  insert(command: City): Observable<void> {
    let url_ = this.baseUrl + "/cities";
    url_ = url_.replace(/[?&]$/, "");
...
update(command: City): Observable<void> {
  let url_ = this.baseUrl + "/cities";
  url_ = url_.replace(/[?&]$/, "");
...
delete(id: number): Observable<void> {
    console.log(id);
  let url_ = this.baseUrl + "/cities/{id}";
