...
import { Nationalities, Nationality } from '@frontend/api-interface';
...
export class NationalitiesService {
  ...
  getAll(pageNumber: number, pageSize: number, filter:string= '',orderBy:string=""): Observable<Nationalities> {
    ...
    let url_ = this.baseUrl + '/nationalities';
    url_ = url_.replace(/[?&]$/, '');
        ...
        _observableCatch((response_: any) => {
          if (response_ instanceof HttpResponseBase) {
            try {
              ...
              return <Observable<Nationalities>>(<any>_observableCatch(e));
            }
          } else
            return <Observable<Nationalities>>(<any>_observableCatch(response_));
        })
      );
  }

  protected processGetAll(response: HttpResponseBase): Observable<Nationalities> {
    ...
    if (status === 200) {
      return servicesLib.blobToText(responseBlob).pipe(
        _observableMergeMap((_responseText) => {
          ...
          result200 = Nationalities.fromJS(resultData200);
    ...
    return _observableOf<Nationalities>(<any>null);
  }

  insert(command: Nationality): Observable<void> {
    let url_ = this.baseUrl + "/nationalities";
    url_ = url_.replace(/[?&]$/, "");
...
update(command: Nationality): Observable<void> {
  let url_ = this.baseUrl + "/nationalities";
  url_ = url_.replace(/[?&]$/, "");
...
delete(id: number): Observable<void> {
    console.log(id);
  let url_ = this.baseUrl + "/nationalities/{id}";
